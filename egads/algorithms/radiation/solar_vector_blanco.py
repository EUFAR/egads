...
class SolarVectorBlanco(egads_core.EgadsAlgorithm):
    ...
    def __init__(self, return_Egads=True):
        ...
        self.metadata = egads_metadata.AlgorithmMetadata({'Inputs':['date-time', 'lat', 'long'],
    ...
    def run(self, date_time, lat, long):

        return egads_core.EgadsAlgorithm.run(self, date_time, lat, long)


    def _algorithm(self, date_time, lat, long):
        ...
        ra = ra % (2 * egads.units.pi)

        # Convert from celestial coordinates to horizontal coordinates
        gmst = 6.6974243242 + 0.0657098283 * n + hour

        lmst = (gmst * 15 + long) * egads.units.pi / 180.0

        hour_angle = lmst - ra

        theta_z = numpy.arccos(numpy.cos(lat * egads.units.pi / 180.0) *
                               numpy.cos(hour_angle) * numpy.cos(delta) +
                               numpy.sin(delta) * numpy.sin(lat * egads.units.pi / 180.0))

        gamma = numpy.arctan2(-numpy.sin(hour_angle), (numpy.tan(delta) *
                                                       numpy.cos(lat * egads.units.pi / 180) -
                                                       numpy.sin(lat * egads.units.pi / 180) *
